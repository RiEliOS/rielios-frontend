...
import {
  TrendingUp, TrendingDown, Wallet, PiggyBank, Target, BarChart3,
  ...
  if (isLoading || !summary) {
    return (
      <div className="p-6 lg:p-8 space-y-6">
        <div className="h-14 w-72 rounded-xl bg-zinc-200 animate-pulse" />
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          {Array.from({ length: 4 }).map((_, i) => <div key={i} className="h-32 rounded-2xl bg-zinc-200 animate-pulse" />)}
        </div>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {Array.from({ length: 3 }).map((_, i) => <div key={i} className="h-52 rounded-2xl bg-zinc-200 animate-pulse" />)}
        </div>
        <div className="h-60 rounded-2xl bg-zinc-200 animate-pulse" />
      </div>
    )
  }
    ...
    <div className="p-6 lg:p-8 space-y-8">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-7 gap-4">
        <StatCard
          ...
          iconBg={netFlow >= 0 ? 'bg-green-100' : 'bg-red-100'}
          accent={netFlow >= 0 ? 'text-green-700' : 'text-red-700'}
        />
        <StatCard
          title="Income"
          ...
          iconBg="bg-emerald-100" accent="text-emerald-700"
        />
        <StatCard
          title="Expenses"
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-4">
        ...
        <div className="bg-white rounded-2xl border border-zinc-200 p-5 space-y-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <span className="text-sm font-bold text-zinc-900">Savings Goals</span>
            </div>
              ...
              <div>
                <div className="flex justify-between text-xs mb-1.5">
                  ...
                  <span className="font-semibold text-zinc-700">
                    ...
                    <span className="text-zinc-400"> / {fmt(s.savings.totalTarget)}</span>
                  </span>
                </div>
                <Progress value={savingsPct} className="h-2" />
                <p className="text-xs text-zinc-400 mt-1">{savingsPct.toFixed(0)}% of total target</p>
              </div>
                    ...
                    return (
                      <div key={g.id}>
                        <div className="flex justify-between text-xs mb-1">
                          <span className="font-medium text-zinc-700 truncate mr-2">{g.name}</span>
                          <span className="text-zinc-400 shrink-0">{pct.toFixed(0)}%</span>
                        </div>
        ...
        <div className="bg-white rounded-2xl border border-zinc-200 p-5 space-y-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <span className="text-sm font-bold text-zinc-900">Investments</span>
            </div>
            ...
            <div className="space-y-4">
              ...
              <div className="flex items-center gap-2 pt-3 border-t border-zinc-100">
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-sm font-bold text-zinc-900 truncate">{fmt(s.investments.totalExpectedReturn)}</p>
                </div>
                <ArrowRight className="h-3.5 w-3.5 text-zinc-300 shrink-0" />
                <div className="flex-1 min-w-0 text-right">
              ...
              {s.goals.goals.map((g) => (
                <div key={g.id}>
                  <div className="flex items-center gap-2 mb-1.5">
                    <span className="text-xs font-medium text-zinc-700 truncate flex-1">{g.title}</span>
        ...
        <div className={cn(
          ...
          s.budgets.overCount > 0 ? 'bg-red-50 border-red-200' : 'bg-white border-zinc-200',
        )}>
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              <div className={cn('h-8 w-8 rounded-lg flex items-center justify-center',
                s.budgets.overCount > 0 ? 'bg-red-100' : 'bg-green-100',
              )}>
                {s.budgets.overCount > 0
                  ? <AlertTriangle className="h-4 w-4 text-red-600" />
                  : <CheckCircle2 className="h-4 w-4 text-green-600" />}
              </div>
              <span className="text-sm font-bold text-zinc-900">Budget Health</span>
            </div>
            {s.budgets.total > 0 && (
              <span className={cn('text-xs px-2 py-0.5 rounded-full font-semibold',
                s.budgets.overCount > 0 ? 'bg-red-100 text-red-700' : 'bg-green-100 text-green-700',
              )}>
            ...
            <div className="space-y-3">
              {s.budgets.items.slice(0, 4).map((b) => {
                ...
                return (
                  <div key={b.id}>
                    <div className="flex justify-between text-xs mb-1">
                      <span className={cn('font-medium truncate mr-2', over ? 'text-red-700' : 'text-zinc-700')}>
                        {b.categoryName}
                      </span>
      ...
      <div className="bg-white rounded-2xl border border-zinc-200 p-5">
        <h2 className="text-sm font-bold text-zinc-900 mb-4">Recent Transactions</h2>
          ...
          <div className="divide-y divide-zinc-100">
            {s.recentTransactions.map((tx) => (
              <div key={tx.id} className="flex items-center gap-3 py-2.5">
                ...
                <span className="text-sm text-zinc-700 flex-1 truncate">{tx.label}</span>
